<div x-data="{
    ...
    <div class="flex flex-col justify-end">
        <h1 class="text-lg font-bold text-right">actu info</h1>
        @if ($errors->any())
        ...
        <div class="flex-col gap-4"
            ...
            <div class="flex items-center justify-center gap-3 by-5">
                <div class="flex flex-1 gap-5 py-5">
                    ...
                    <textarea name="h" id="descri" wire:model="descri" class="flex-1 p-2 placeholder-gray-600 transition transform border rounded-lg" placeholder="descri"></textarea>
                </div>
            ...
            <div class="grid flex-1 grid-cols-4 gap-3 mt-3">
                ...
                @foreach ($actus as $act)
                    <a class="flex flex-col justify-center p-2 text-center shadow" wire:click="charger({{$act}})">
                        @if (Storage::exists('public/actu/'.$act->id.'.png'))    
                            <img src="{{asset('storage/actu/'.$act->id.'.png')}}?{{ rand() }}" alt="Pas d'image pour cette info" srcset="" class="flex-1">
                        @endif
                        <h1 class="text-lg font-bold ">{{$act->titre}}</h1>
