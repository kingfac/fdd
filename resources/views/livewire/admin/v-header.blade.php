<div x-data="{
    ...
    <div class="flex flex-col justify-end">
        <h1 class="text-lg font-bold text-right">header info</h1>
        @if ($errors->any())
        ...
        <div class="flex-col gap-4"
            ...
            <div class="flex items-end justify-end gap-3 by-5">
                <div class="flex-1 w-full">
                    <p class="pb-2"><b>lib</b></p>
                    <input type="text" placeholder="lib de l'image" class="w-full px-1 py-2 border rounded-lg" wire:model="lib">
                </div>
                <div class="flex-1">
            ...
            <div class="grid flex-1 grid-cols-5 gap-5 mt-3">
                ...
                @endif
                @foreach ($headers as $gal)
                    <a class="flex flex-col justify-center p-2 text-center shadow" wire:click="charger({{$gal}})">
                        @if (Storage::exists('public/header/'.$gal->id.'.png'))    
                            <img src="{{asset('storage/header/'.$gal->id.'.png')}}?{{ rand() }}" alt="Pas d'image pour cette info" srcset="" class="flex-1">
                        @endif
                        <h1 class="text-lg font-bold ">{{$gal->lib}}</h1>
                    </a>
                @endforeach
            </div>
        </div>
