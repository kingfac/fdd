<div x-data="{
    ...
    <div class="flex flex-col justify-end">
        <h1 class="text-lg font-bold text-right">Comptes / Banque</h1>
        @if ($errors->any())
            ...
            <div class="flex items-center justify-center gap-3 by-5">
                <div class="flex flex-1 gap-5 py-5">
                    <div class="flex flex-col flex-1 gap-3">
                        <input type="text" class="flex-1 w-full p-2 placeholder-gray-700 border-2 rounded-lg" placeholder="Nom" id="nom" wire:model="nom">
                        <input name="" id="" wire:model="numero" class="flex-1 w-full p-2 placeholder-gray-600 transition transform border rounded-lg" placeholder="numero" />
                ...
                <div class="grid grid-cols-2 gap-4 bg-white">
                    ...
                    <button class="text-lg font-bold text-white bg-gray-500 -3" wire:click="resetFields" @click="descri='';nom='';id=0">Clear</button>        
                </div>
            </div>
            <div class="grid flex-1 grid-cols-4 gap-3 mt-3">
                @if ($photo)
                ...
                @foreach ($comptes as $cpt)
                    <a class="flex flex-col justify-center p-2 text-center shadow" wire:click="charger({{$cpt}})">
                        @if (Storage::exists('public/compte/'.$cpt->id.'.png'))    
                            <img src="{{asset('storage/compte/'.$cpt->id.'.png')}}?{{ rand() }}" alt="Pas d'image pour cette info" srcset="" class="flex-1">
                        @endif
                        <h1 class="text-lg font-bold text-left">Nom : {{$cpt->nom}}</h1>
                        <p class="text-left ">
                            <b>numero : </b>{{$cpt->numero}}
                        </p>
                    </a>
                @endforeach
            </div>
