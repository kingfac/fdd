<div x-data="{
    ...
    <div class="px-5 py-5 ">
        <div class="flex flex-col gap-4 divide-y-2">
            <div class="p-2">   
                <h1 class="text-xl font-bold text-right divide-x-4 divide-gray-300">Contact</h1>
                <div class="flex flex-col py-5">
                    <div class="flex flex-1 gap-5 py-5">
                        <div class="flex flex-col flex-1 gap-3">
                            <div class="flex-1 w-full p-2 placeholder-gray-600 transition transform border rounded-lg">
                                <label for="type">Type</label>
                                <select name="type" id="type" wire:model="type" class="flex-1 w-full p-2 placeholder-gray-600 transition transform border rounded-lg">
                                    <option value="tel">TELEPHONE</option>
                                    <option value="mailto">EMAIL</option>
                                    <option value="null">AUTRES</option>
                                </select>
                            </div>
                            <input name="" id="" wire:model="lib" class="flex-1 w-full p-2 placeholder-gray-600 transition transform border rounded-lg" placeholder="lib" />
                            <input name="" id="" wire:model="icon" class="flex-1 w-full p-2 placeholder-gray-600 transition transform border rounded-lg" placeholder="Icone" />
                        </div>
                        <div class="grid grid-cols-2 gap-2">
                            <button class="flex-1 px-3 py-2 text-lg font-bold text-white bg-green-600" @click="modal=true;save=true;update=false;del=false">Enregistrer</button>        
                            ...
                            <button class="flex-1 text-lg font-bold text-white bg-gray-500 -3" wire:click="resetFields" @click="contenu='';titre='';id=0">Clear</button>        
                        </div>
                    </div>
            ...
            <div class="flex flex-col gap-2 divide-y-2">
                @foreach ($contacts as $ligne)
                    <div class="flex flex-col">
                        <button class="flex justify-between p-2 font-bold text-white transition duration-300 bg-green-600 shadow cursor-pointer hover:bg-green-800" wire:click="charger({{$ligne}})">
                            <h1><?php echo $ligne->type; ?></h1>
    ...
    <div class="absolute top-0 left-0 flex items-center justify-center w-screen h-screen gap-10 p-5 text-white black-transparent" x-show="modal" @click="modal=false;" x-transition.500ms>
        <div class="flex flex-col items-center justify-center gap-10 p-5 black-transparent" >
            ...
            <div class="flex w-full p-2">
                <button class="flex-1 px-3 py-2 text-lg font-bold text-white bg-green-600 hover:bg-green-700"   x-show="save" wire:click="store">Enregistrer</button>        
