<div class="mt-12">
    <div class="w-full h-full splide">
         <div class="splide__arrows">
             <button class="text-3xl text-green-500 splide__arrow splide__arrow--prev">
                <
             </button>
             <button class="text-3xl text-green-500 splide__arrow splide__arrow--next ">
                > 
             </button>
         </div>
         <div class="splide__track">
               <ul class="splide__list">  
                   <li class="splide__slide" style="height: 50vh;">
                       <img src="{{asset('assets/87C095A4-F69F-42CA-9FD5-6E7A17E8EA2B.jpeg')}}" alt="" srcset="" class="w-full h-full">
                   </li>
                   <li class="splide__slide">
                    <img src="{{asset('assets/accompagner-ministeres-developpement-rural-projet-cameroun.jpg')}}" alt="" srcset="" class="w-full">
                    </li>
                    <li class="splide__slide">
                        <img src="{{asset('assets/20200616-DRC-01-780x439.jpg')}}" alt="" srcset="" class="w-full">
                    </li>
               </ul>
         </div>
    </div>
    <div class="absolute flex flex-col top-12" style="height: 50vh;">
        <div class="flex-1 pt-5 pl-5">
            <img src="{{asset('assets/logo/logo fdd white.png')}}" alt="" srcset="" class="h-24 ">
        </div>
        <div class="flex items-end justify-end flex-1">
            <p class="px-10 py-2 font-bold text-white bg-green-600">Formateurs pour un Développement Durable</p>
        </div>
    </div>
</div>
